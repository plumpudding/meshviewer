define(function () {
  return function (config) {
    ...
    self.setData = function (d) {
      ...
      var totalUplinks = sum(d.nodes.all.filter(online).filter( function (d) {
        var rt = false
      ...
      var totalHotspots = sum(d.nodes.all.filter(online).filter( function (d) {
        var rt = true
        d.neighbours.forEach(function (c) {
          if(!c.link.vpn)
            rt = false
      ...
      stats.textContent = totalNodes + " Zugangspunkte " +
                          "(" + nodetext + "), " +
                          totalClients + " Clients, " +
                          totalUplinks + " Uplinks, " +
                          (totalNodes - totalHotspots) + " Mesh-Knoten, " +
                          (totalHotspots - totalGateways) + " HotSpots, " +
