define(["map/clientlayer", "map/labelslayer",
  ...
  function (ClientLayer, LabelsLayer, d3, L, moment, LocationMarker, rbush) {
    ...
    var SearchLocateButton = L.Control.extend({
        options: {
          position: "bottomright"
        },
        ...
        onAdd: function () {
          var button = L.DomUtil.create("button", "search-locate-user")
          button.textContent = ""
    ...
    return function (config, linkScale, sidebar, router, buttons) {
      ...
      var searchLocateUserButton = new SearchLocateButton(function (d){
        if (d) {
              ...
              marker = new L.Marker(map.getCenter(), {icon: icon, draggable: "true"}).addTo(map)
              router.geo(marker.getLatLng())
          ...
          searchLocateUserButton.set(false)
        }
      })
      ...
      addButton(searchLocateUserButton)
      ...
      self.gotoGeo = function (d) {
        highlight = {type: "geo", o: d}
        //updateView()
      }
