...
function (moment, Router, L, GUI, numeral) {
  return function (config) {
    function handleData(data) {
      ...
      var now = moment()
      var ageNew = moment(now).subtract(config.maxAgeOnline, "days")
      var ageOffline = moment(now).subtract(config.maxAgeOffline, "days")

      var newnodes = limit("firstseen", ageNew, sortByKey("firstseen", nodes).filter(online))
      var lostnodes = limit("lastseen", ageOffline, sortByKey("lastseen", nodes).filter(offline))
